...
, mSceneGraph()
, mSceneLayers()
, mWorldBounds(sf::Vector2f(0.f, 0.f), sf::Vector2f(mWorldView.getSize().x, 2000.f))
...
{
	// Scroll the world
	mWorldView.move(sf::Vector2f(0.f, mScrollSpeed * dt.asSeconds()));
...
void World::buildScene()
{
	...
	leftEscort->setPosition(sf::Vector2f(-80.f, 50.f));
	mPlayerAircraft->attachChild(std::move(leftEscort));

	std::unique_ptr<Aircraft> rightEscort(new Aircraft(Aircraft::Raptor, mTextures));
	rightEscort->setPosition(sf::Vector2f(80.f, 50.f));
